...
object Task4 : Task() {
   ...
   override fun runTask() {
      ...
      printlnDouble(Intergate.trapeze(Functions::withSinus, .0, 4.5, .01))
      println("Funkcja obliczana ze stałym krokiem dx = 0.001")
      printlnDouble(Intergate.trapeze(Functions::withSinus, .0, 4.5, .001))
      ...
      do {
         step /= 10
         val result = Intergate.trapeze(Functions::withSinus, .0, 4.5, step)
      ...
      println("Funkcja obliczana ze stałym krokiem dx = 0.1")
      printlnDouble(Intergate.simpson(Functions::withSinus, .0, 4.5, .1))
      println("Funkcja obliczana ze stałym krokiem dx = 0.01")
      printlnDouble(Intergate.simpson(Functions::withSinus, .0, 4.5, .01))
      println("Funkcja obliczana ze stałym krokiem dx = 0.001")
      printlnDouble(Intergate.simpson(Functions::withSinus, .0, 4.5, .001))
      printThinSeparator()
      ...
      do {
         step /= 10
         val result = Intergate.simpson(Functions::withSinus, .0, 4.5, step)
         val diff = abs(expectedValue-result)
         println("Różnica wartści dla wielkości kroku $step wynosi $diff")
      } while (diff >= epsilon)
